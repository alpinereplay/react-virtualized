...
export default class Table extends React.PureComponent {
  ...
  render() {
    ...
    return <div aria-label={this.props['aria-label']} aria-labelledby={this.props['aria-labelledby']} aria-colcount={React.Children.toArray(children).length} aria-rowcount={this.props.rowCount} className={clsx('ReactVirtualized__Table', className)} id={id} role="grid" style={style}>
  ...
  _createColumn({
    ...
    return <div aria-colindex={columnIndex + 1} aria-describedby={id} className={clsx('ReactVirtualized__Table__rowColumn', className)} key={'Row' + rowIndex + '-' + 'Col' + columnIndex} onClick={onClick} role="gridcell" style={style} title={title}>
        {renderedCell}
      </div>;
  }
  ...
  _createHeader({
    ...
    if (sortEnabled || onHeaderClick) {
      ...
      const newSortDirection = isFirstTimeSort ? defaultSortDirection : sortDirection === SortDirection.DESC ? SortDirection.ASC : SortDirection.DESC;
    ...
    // Since it results in an extra method call to 'babel-runtime/helpers/extends'
    // See PR https://github.com/bvaughn/react-virtualized/pull/942
    return <div aria-label={headerAriaLabel} aria-sort={headerAriaSort} className={classNames} id={id} key={'Header-Col' + index} onClick={headerOnClick} onKeyDown={headerOnKeyDown} role="columnheader" style={style} tabIndex={headerTabIndex}>
        {renderedHeader}
      </div>;
  }
