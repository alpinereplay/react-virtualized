...
import type { CellRenderer, CellRangeRenderer, CellPosition, CellSize, CellSizeGetter, NoContentRenderer, Scroll, ScrollbarPresenceChange, RenderedSection, OverscanIndicesGetter, Alignment, CellCache, StyleCache } from "./types";
...
class Grid extends React.PureComponent<Props, State> {
  ...
  scrollToCell({
    ...
    if (columnCount > 1 && columnIndex !== undefined) {
      this._updateScrollLeftForScrollToColumn({ ...props,
        scrollToColumn: columnIndex
      });
    }

    if (rowIndex !== undefined) {
      this._updateScrollTopForScrollToRow({ ...props,
        scrollToRow: rowIndex
      });
    }
  }
  ...
  componentDidUpdate(prevProps: Props, prevState: State) {
    ...
    const columnOrRowCountJustIncreasedFromZero = columnCount > 0 && prevProps.columnCount === 0 || rowCount > 0 && prevProps.rowCount === 0;
  ...
  render() {
    ...
    return <div ref={this._setScrollingContainerRef} {...containerProps} aria-label={this.props['aria-label']} aria-readonly={this.props['aria-readonly']} className={clsx('ReactVirtualized__Grid', className)} id={id} onScroll={this._onScroll} role={role} style={{ ...gridStyle,
      ...style
    }} tabIndex={tabIndex}>
        {childrenToDisplay.length > 0 && <div className="ReactVirtualized__Grid__innerScrollContainer" role={containerRole} style={{
