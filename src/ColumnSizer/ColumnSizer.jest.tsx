...
describe('ColumnSizer', () => {
  function getMarkup({
    ...
    return <ColumnSizer columnMinWidth={columnMinWidth} columnMaxWidth={columnMaxWidth} columnCount={columnCount} width={width}>
        {({
        adjustedWidth,
        columnWidth,
        registerChild
      }) => <div>
            <Grid columnCount={columnCount} columnWidth={columnWidth} height={50} ref={registerChild} cellRenderer={cellRenderer} rowHeight={50} rowCount={1} width={adjustedWidth} />
            <div className="debug">
  ...
  it('should error if the registered child is not a Grid or a MultiGrid', () => {
    spyOn(console, 'error');
    expect(() => {
      render(<ColumnSizer columnMinWidth={100} columnMaxWidth={100} columnCount={100} width={100}>
          {({
          registerChild
