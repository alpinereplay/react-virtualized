...
describe('InfiniteLoader', () => {
  ...
  function getMarkup({
    ...
    return <InfiniteLoader isRowLoaded={isRowLoaded} loadMoreRows={loadMoreRows} minimumBatchSize={minimumBatchSize} rowCount={rowCount} threshold={threshold}>
        {({
        onRowsRendered,
        registerChild
      }) => {
        innerOnRowsRendered = onRowsRendered;
        return <List ref={registerChild} height={height} onRowsRendered={onRowsRendered} rowHeight={rowHeight} rowRenderer={rowRenderer} rowCount={rowCount} scrollToIndex={scrollToIndex} width={width} />;
      }}
      </InfiniteLoader>;
  }
