...
function renderHelper({
  ...
  render(<CellMeasurer cache={cache} columnIndex={0} parent={parent} rowIndex={0} style={{}}>
      {children}
    </CellMeasurer>);
}
...
describe('CellMeasurer', () => {
  ...
  it('should explicitly set width/height style to "auto" before re-measuring', () => {
    ...
    const child = jest.fn().mockReturnValue(<div style={{
      width: 100,
      height: 30
    }} />);
    let measurer;
    const node = findDOMNode(render(<CellMeasurer ref={ref => {
      measurer = ref;
    }} cache={cache} columnIndex={0} parent={parent} rowIndex={0} style={{}}>
          {child}
        </CellMeasurer>));
    ...
    const {
      height,
      width
    } = measurer._getCellMeasurements(node);
  ...
  it('should reset width/height style values after measuring with style "auto"', () => {
    ...
    const node = findDOMNode(render(<CellMeasurer cache={cache} columnIndex={0} parent={parent} rowIndex={0} style={{}}>
          {child}
        </CellMeasurer>));
