...
const calculateSizeAndPositionDataAndUpdateScrollOffset_1 = __importDefault(require("./utils/calculateSizeAndPositionDataAndUpdateScrollOffset"));
const ScalingCellSizeAndPositionManager_1 = __importDefault(require("./utils/ScalingCellSizeAndPositionManager"));
const createCallbackMemoizer_1 = __importDefault(require("../utils/createCallbackMemoizer"));
const defaultOverscanIndicesGetter_1 = __importStar(require("./defaultOverscanIndicesGetter"));
const updateScrollIndexHelper_1 = __importDefault(require("./utils/updateScrollIndexHelper"));
const defaultCellRangeRenderer_1 = __importDefault(require("./defaultCellRangeRenderer"));
...
class Grid extends React.PureComponent {
    ...
    _onGridRenderedMemoizer = (0, createCallbackMemoizer_1.default)();
    ...
    constructor(props) {
        super(props);
        const columnSizeAndPositionManager = new ScalingCellSizeAndPositionManager_1.default({
            cellCount: props.columnCount,
            cellSizeGetter: (params) => Grid._wrapSizeGetter(props.columnWidth)(params),
            estimatedCellSize: Grid._getEstimatedColumnSize(props),
        });
        const rowSizeAndPositionManager = new ScalingCellSizeAndPositionManager_1.default({
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        let maybeStateA;
        let maybeStateB;
        (0, calculateSizeAndPositionDataAndUpdateScrollOffset_1.default)({
        ...
        (0, calculateSizeAndPositionDataAndUpdateScrollOffset_1.default)({
            ...
            nextCellSize: typeof nextProps.rowHeight === "number"
                ? nextProps.rowHeight
                : null,
    ...
    render() {
        ...
        return (React.createElement("div", { ref: this._setScrollingContainerRef, ...containerProps, "aria-label": this.props["aria-label"], "aria-readonly": this.props["aria-readonly"], className: "ReactVirtualized__Grid", id: id, onScroll: this._onScroll, role: role, style: { ...gridStyle, ...style }, tabIndex: tabIndex ?? undefined },
            childrenToDisplay.length > 0 && (React.createElement("div", { className: "ReactVirtualized__Grid__innerScrollContainer", role: containerRole, style: {
    ...
    _calculateChildrenToRender(props = this.props, state = this.state) {
        ...
        const scrollTop = this._initialScrollTop > 0
            ? this._initialScrollTop
            : state.scrollTop;
        const scrollLeft = this._initialScrollLeft > 0
        ...
        if (height > 0 && width > 0) {
            ...
            this._renderedColumnStartIndex = visibleColumnIndices.start ?? 0;
            this._renderedColumnStopIndex = visibleColumnIndices.stop ?? 0;
            this._renderedRowStartIndex = visibleRowIndices.start ?? 0;
            this._renderedRowStopIndex = visibleRowIndices.stop ?? 0;
