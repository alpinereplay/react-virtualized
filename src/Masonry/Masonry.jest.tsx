...
function assertVisibleCells(rendered, text) {
  expect(Array.from(rendered.querySelectorAll('.cell')).map(node => node.textContent).sort().join(',')).toEqual(text);
}
...
function createCellRenderer(cache, renderCallback) {
  ...
  return function cellRenderer({
    ...
    return <CellMeasurer cache={cache} index={index} key={key} parent={parent}>
...
describe('Masonry', () => {
  ...
  describe('layout and measuring', () => {
    ...
    it('should consider scroll only of the container element and not of any ancestor element', () => {
      ...
      const renderScrollableCell = index => <div style={{
        height: '50px',
        overflow: 'visible'
  ...
  describe('keyMapper', () => {
    it('should pass the correct key to rendered cells', () => {
      ...
      const cellRenderer = jest.fn().mockImplementation(({
        index,
        key,
        style
