...
export default class Table extends React.PureComponent {
    static propTypes = {
        ...
        children: (props: any): Error | null => {
            const children = React.Children.toArray(props.children)

            for (let i = 0; i < children.length; i++) {
                const childType = (children[i] as any).type
                ...
                if (
                    ...
                    return new Error(
                        "Table only accepts children of type Column"
                    )
                }
            }
    ...
    getOffsetForRow({ alignment, index }: any) {
        if (this.Grid) {
    ...
    getScrollbarWidth() {
        if (this.Grid) {
            const Grid: any = findDOMNode(this.Grid)
    ...
    render() {
        ...
        const { scrollbarWidth } = this.state as any
        ...
        React.Children.toArray(children).forEach((column, index) => {
            let style = (column as any).props.style
        ...
        const ariaLabel = (this.props as any).ariaLabel || ""
        const ariaLabeledBy = (this.props as any).ariaLabeledBy || null
        const rowCount = (this.props as any).rowCount || 0
        return (
            <div
    ...
    _createColumn({
        ...
        const { onColumnClick } = this.props as any
        const {
            cellDataGetter,
            cellRenderer,
            className,
    ...
    _createHeader({ column, index }: any) {
        const {
            headerClassName,
            headerStyle,
            onHeaderClick,
    ...
    _createRow({ rowIndex: index, isScrolling, key, parent, style }: any) {
        const {
            children,
            onRowClick,
            onRowDoubleClick,
     ...
     */
    _getFlexStyleForColumn(column: any, customStyle: any = {}) {
    ...
    _getHeaderColumns() {
        const { children, disableHeader } = this.props as any
    ...
    _getRowHeight(rowIndex: number) {
        const { rowHeight } = this.props as any
    ...
    _onSectionRendered({
        ...
        const { onRowsRendered } = this.props as any
        onRowsRendered({
