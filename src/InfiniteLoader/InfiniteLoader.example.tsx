...
export default class InfiniteLoaderExample extends React.PureComponent {
  ...
  render() {
    ...
    return <ContentBox>
        <ContentBoxHeader text="InfiniteLoader" sourceLink="https://github.com/bvaughn/react-virtualized/blob/master/source/InfiniteLoader/InfiniteLoader.example.js" docsLink="https://github.com/bvaughn/react-virtualized/blob/master/docs/InfiniteLoader.md" />
        ...
        <InfiniteLoader isRowLoaded={this._isRowLoaded} loadMoreRows={this._loadMoreRows} rowCount={list.size}>
          ...
          registerChild
              ...
              {({
            width
          }) => <List ref={registerChild} className={styles.List} height={200} onRowsRendered={onRowsRendered} rowCount={list.size} rowHeight={30} rowRenderer={this._rowRenderer} width={width} />}
            </AutoSizer>}
        </InfiniteLoader>
      </ContentBox>;
  }
  ...
  _rowRenderer({
    ...
    if (loadedRowsMap[index] === STATUS_LOADED) {
      ...
      content = <div className={styles.placeholder} style={{
        width: row.size
      }} />;
    }
