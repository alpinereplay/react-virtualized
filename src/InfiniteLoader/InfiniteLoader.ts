...
export default class InfiniteLoader extends React.PureComponent {
    ...
    _registeredChild: any
    _lastRenderedStartIndex: number = 0
    _lastRenderedStopIndex: number = 0
    ...
    resetLoadMoreRowsCache(autoReload: any) {
        this._loadMoreRowsMemoizer = createCallbackMemoizer()
    ...
    _loadUnloadedRanges(unloadedRanges: any[]) {
        const { loadMoreRows } = this.props as any
    ...
    _onRowsRendered({ startIndex, stopIndex }: any) {
    ...
    _doStuff(startIndex: number, stopIndex: number) {
        ...
        let ranges = [...unloadedRanges].map((range: any) => {
            return [range.startIndex, range.stopIndex]
        })
        const squashedUnloadedRanges = ([] as any[]).concat(ranges)
    ...
    _registerChild(registeredChild: any) {
