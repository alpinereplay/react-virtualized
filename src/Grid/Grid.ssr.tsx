...
test('should render Grid with dom server', () => {
  const rendered = ReactDOMServer.renderToString(<Grid cellRenderer={({
    style,
    key,
    rowIndex,
    columnIndex
        ...
        </div>} columnCount={1000} columnWidth={20} height={500} rowCount={1000} rowHeight={20} width={500} />);
...
test('should support :scrollToColumn and :scrollToRow in server render', () => {
  const rendered = ReactDOMServer.renderToString(<Grid cellRenderer={({
    ...
    columnIndex
          ...
          {rowIndex + ':' + columnIndex}
        </div>} columnCount={1000} columnWidth={20} scrollToColumn={250} height={500} rowCount={1000} rowHeight={20} scrollToRow={250} width={500} />);
