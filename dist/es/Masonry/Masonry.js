...
const PositionCache_1 = __importDefault(require("./PositionCache"));
...
class Masonry extends React.PureComponent {
    ...
    _positionCache = new PositionCache_1.default();
    _startIndex = null;
    ...
    _scrollingContainer;
    clearCellPositions() {
        this._positionCache = new PositionCache_1.default();
        this.forceUpdate();
    }
    ...
    invalidateCellSizeAfterRender({ rowIndex: index }) {
        if (this._invalidateOnUpdateStartIndex === null) {
            ...
            this._invalidateOnUpdateStartIndex = Math.min(this._invalidateOnUpdateStartIndex ?? index, index);
            this._invalidateOnUpdateStopIndex = Math.max(this._invalidateOnUpdateStopIndex ?? index, index);
        }
    }
    ...
    componentWillUnmount() {
        if (this._debounceResetIsScrollingId) {
            (0, requestAnimationTimeout_1.cancelAnimationTimeout)(this._debounceResetIsScrollingId);
        }
    }
    render() {
        ...
        if (shortestColumnSize < scrollTop + height + overscanByPixels &&
            ...
            const batchSize = Math.min(cellCount - measuredCellCount, Math.ceil((((scrollTop +
                height +
                overscanByPixels -
                shortestColumnSize) /
        ...
        return (React.createElement("div", { ref: this._setScrollingContainerRef, "aria-label": ariaLabel, className: "ReactVirtualized__Masonry", id: id ?? "", onScroll: this._onScroll, role: role, style: {
            ...
            React.createElement("div", { className: "ReactVirtualized__Masonry__innerScrollContainer", style: {
                    width: "100%",
    ...
    _debounceResetIsScrolling() {
        ...
        this._debounceResetIsScrollingId = (0, requestAnimationTimeout_1.requestAnimationTimeout)(this._debounceResetIsScrollingCallback, scrollingResetTimeInterval);
    }
