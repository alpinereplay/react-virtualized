...
describe('Grid', () => {
  function defaultCellRenderer({
    ...
    return <div className="gridItem" key={key} style={style}>
  ...
  function getMarkup(props = {}) {
    return <Grid cellRenderer={defaultCellRenderer} columnCount={NUM_COLUMNS} columnWidth={DEFAULT_COLUMN_WIDTH} getScrollbarSize={getScrollbarSize0} height={DEFAULT_HEIGHT} overscanColumnCount={0} overscanRowCount={0} autoHeight={false} rowHeight={DEFAULT_ROW_HEIGHT} rowCount={NUM_ROWS} width={DEFAULT_WIDTH} {...props} />;
  }
  ...
  describe('number of rendered children', () => {
    ...
    it('should not render/parent cells that are null or false', () => {
      function cellRenderer({
        ...
        if (columnIndex === 0) {
          ...
          return <div className="cell" key={key} style={style}>
              {`row:${rowIndex}, column:${columnIndex}`}
            </div>;
        }
      }
    ...
    it('should scroll to the last existing point when rows are removed', () => {
      ...
      const updatedGrid = render(getMarkup({
        rowCount: 10
      }));
  ...
  describe('DEV warnings', () => {
    ...
    it('should warn about CellMeasurer measured cells that forget to include the :style property', () => {
      ...
      const cellRenderer = jest.fn();
      cellRenderer.mockImplementation(params => <CellMeasurer cache={cache} columnIndex={params.columnIndex} key={params.key} parent={params.parent} rowIndex={params.rowIndex} style={params.style}>
          <div />
        </CellMeasurer>);
      render(getMarkup({
        cellRenderer,
