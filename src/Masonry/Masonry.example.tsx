...
export default class GridExample extends React.PureComponent {
  ...
  render() {
    ...
    if (windowScrollerEnabled) {
      child = <WindowScroller overscanByPixels={overscanByPixels}>
    ...
    return <ContentBox>
        <ContentBoxHeader text="Masonry" sourceLink="https://github.com/bvaughn/react-virtualized/blob/master/source/Masonry/Masonry.example.js" docsLink="https://github.com/bvaughn/react-virtualized/blob/master/docs/Masonry.md" />
        ...
        <ContentBoxParagraph>
          <label className={styles.checkboxLabel}>
            <input aria-label="Use WindowScroller?" checked={windowScrollerEnabled} className={styles.checkbox} type="checkbox" onChange={event => {
        ...
        <InputRow>
          <LabeledInput label="Height" name="height" onChange={event => {
          this.setState({
            height: parseInt(event.target.value, 10) || 300
          });
        }} value={height} />
          <LabeledInput label="Column Width" name="columnWidth" onChange={event => {
          this._cache.clearAll();
          ...
          <LabeledInput label="Gutter Size" name="gutterSize" onChange={event => {
          this.setState({
          ...
          <LabeledInput label="Overscan (px)" name="overscanByPixels" onChange={event => {
          this.setState({
  ...
  _cellRenderer({
    ...
    return <CellMeasurer cache={this._cache} index={index} key={key} parent={parent}>
  ...
  _renderAutoSizer({
    ...
    return <AutoSizer disableHeight height={height} onResize={this._onResize} overscanByPixels={overscanByPixels} scrollTop={this._scrollTop}>
        {this._renderMasonry}
      </AutoSizer>;
  }
