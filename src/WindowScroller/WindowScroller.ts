...
export default class WindowScroller extends React.PureComponent<Props, State> {
    ...
    _registerChild = (element: Element | null | undefined) => {
    ...
    _onChildScroll = ({ scrollTop }: any) => {
        if (this.state.scrollTop === scrollTop) {
            return
        }

        const { scrollElement } = this.props as any
    ...
    _registerResizeListener = (element: any) => {
        if (element === window) {
    ...
    _unregisterResizeListener = (element: any) => {
        if (element === window) {
    ...
    __handleWindowScrollEvent = () => {
        ...
        if (scrollElement) {
            ...
            const scrollTop = Math.max(
                0,
                scrollOffset.top - this._positionFromTop
            )
            this.setState({
                isScrolling: true,
                scrollLeft,
